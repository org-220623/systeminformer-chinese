...
{
    COLOR_ITEM(L"ColorOwnProcesses", L"当前用户进程", L"与 System Informer 使用同一用户帐户运行的进程。"),
    COLOR_ITEM(L"ColorSystemProcesses", L"系统进程", L"以 NT AUTHORITY\\SYSTEM 用户帐户运行的进程。"),
...
#ifdef _WIN64
    COLOR_ITEM(L"ColorWow64Processes", L"32 位进程", L"在 WOW64 (即 32 位) 下运行的进程。"),
#endif
    COLOR_ITEM(L"ColorDebuggedProcesses", L"被调试进程", L"当前正在调试的进程。"),
    COLOR_ITEM(L"ColorElevatedProcesses", L"优先级提升进程", L"在启用了 UAC 的系统上具有完全权限的进程。"),
    COLOR_ITEM(L"ColorUIAccessProcesses", L"UIAccess 进程", L"具有 UIAccess 权限的进程。"),
    COLOR_ITEM(L"ColorPicoProcesses", L"Pico 进程", L"属于适用于 Linux 的 Windows 子系统 (WSL) 的进程。"),
    COLOR_ITEM(L"ColorImmersiveProcesses", L"UWP 进程和 DLL", L"属于 Modern UI 应用的进程和 DLL。"),
    COLOR_ITEM(L"ColorSuspended", L"已挂起的进程和线程", L"已暂停执行的进程和线程。"),
    COLOR_ITEM(L"ColorPartiallySuspended", L"部分挂起的进程和线程", L"部分暂停执行的进程和线程。"),
    COLOR_ITEM(L"ColorDotNet", L".NET 进程和 DLL", L".NET (托管) 进程和 DLL。"),
    COLOR_ITEM(L"ColorPacked", L"打包的进程", L"可执行文件有时会被 \"打包\" 以减小其大小。"),
    COLOR_ITEM(L"ColorLowImageCoherency", L"低进程映像一致性", L"与映射映像相比，支持进程的映像文件一致性较低。"),
    COLOR_ITEM(L"ColorGuiThreads", L"GUI 线程", L"至少进行过一次与 GUI 相关的系统调用的线程。"),
    COLOR_ITEM(L"ColorRelocatedModules", L"重定位 DLL", L"未加载到其首选映像基础的 DLL。"),
    COLOR_ITEM(L"ColorProtectedHandles", L"受保护句柄", L"受保护无法关闭的句柄。"),
    COLOR_ITEM(L"ColorProtectedProcess", L"受保护进程", L"具有内置保护级别的进程。"),
    COLOR_ITEM(L"ColorInheritHandles", L"可继承句柄", L"可被子进程继承的句柄。"),
    COLOR_ITEM(L"ColorHandleFiltered", L"已过滤进程", L"受句柄对象回调保护的进程。"),
    COLOR_ITEM(L"ColorUnknown", L"不受信任的 DLL 和服务", L"未进行数字签名的服务和 DLL。"),
    COLOR_ITEM(L"ColorServiceDisabled", L"已禁用的服务", L"已禁用的服务。"),
    //COLOR_ITEM(L"ColorServiceStop", L"已停止的服务", L"未运行的服务。")
    COLOR_ITEM(L"ColorEfficiencyMode", L"电源节能", L"设置节能模式的进程和线程。"),
};
