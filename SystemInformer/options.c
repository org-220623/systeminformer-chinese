            ...
            if (programFilesPath = PhExpandEnvironmentStringsZ(L"%ProgramFiles%\\"))
            {
                if (!PhStartsWithString(applicationFileName, programFilesPath, TRUE))
                {
                    if (PhShowMessage2(
                        ...
                        L"从非安全位置 (例如 Program Files 文件夹) 运行 System Informer 时，不建议使用 %s。\r\n\r\n确定要继续吗?",
                        Message
                        ) == IDNO)
                    {
...
#define COLOR_ITEM(SettingName, Name, Description) { SettingName, L"使用" SettingName, Name, Description }

static COLOR_ITEM ColorItems[] =
{
    COLOR_ITEM(L"ColorOwnProcesses", L"Own processes", L"与 System Informer 使用同一用户帐户运行的进程。"),
    COLOR_ITEM(L"ColorSystemProcesses", L"System processes", L"以 NT AUTHORITY\\SYSTEM 用户帐户运行的进程。"),
    COLOR_ITEM(L"ColorServiceProcesses", L"Service processes", L"承载一个或多个服务的进程。"),
    COLOR_ITEM(L"ColorBackgroundProcesses", L"Background processes", L"具有后台调度优先级的进程。"),
    COLOR_ITEM(L"ColorJobProcesses", L"Job processes", L"与作业关联的进程。"),
#ifdef _WIN64
    COLOR_ITEM(L"ColorWow64Processes", L"32-bit processes", L"在 WOW64 (即 32 位) 下运行的进程。"),
#endif
    COLOR_ITEM(L"ColorDebuggedProcesses", L"Debugged processes", L"当前正在调试的进程。"),
    COLOR_ITEM(L"ColorElevatedProcesses", L"Elevated processes", L"在启用了 UAC 的系统上具有完全权限的进程。"),
    COLOR_ITEM(L"ColorUIAccessProcesses", L"UIAccess processes", L"具有 UIAccess 权限的进程。"),
    COLOR_ITEM(L"ColorPicoProcesses", L"Pico processes", L"属于适用于 Linux 的 Windows 子系统 (WSL) 的进程。"),
    COLOR_ITEM(L"ColorImmersiveProcesses", L"Immersive processes and DLLs", L"属于 Modern UI 应用的进程和 DLL。"),
    COLOR_ITEM(L"ColorSuspended", L"Suspended processes and threads", L"已暂停执行的进程和线程。"),
    COLOR_ITEM(L"ColorPartiallySuspended", L"Partially suspended processes and threads", L"部分暂停执行的进程和线程。"),
    COLOR_ITEM(L"ColorDotNet", L".NET processes and DLLs", L".NET (托管) 进程和 DLL。"),
    COLOR_ITEM(L"ColorPacked", L"Packed processes", L"可执行文件有时会被 \"打包\" 以减小其大小。"),
    COLOR_ITEM(L"ColorLowImageCoherency", L"Low process image coherency", L"与映射映像相比，支持进程的映像文件一致性较低。"),
    COLOR_ITEM(L"ColorGuiThreads", L"GUI threads", L"至少进行过一次与 GUI 相关的系统调用的线程。"),
    COLOR_ITEM(L"ColorRelocatedModules", L"Relocated DLLs", L"未加载到其首选映像基础的 DLL。"),
    COLOR_ITEM(L"ColorProtectedHandles", L"Protected handles", L"受保护无法关闭的句柄。"),
    COLOR_ITEM(L"ColorProtectedProcess", L"Protected processes", L"具有内置保护级别的进程。"),
    COLOR_ITEM(L"ColorInheritHandles", L"Inheritable handles", L"可被子进程继承的句柄。"),
    COLOR_ITEM(L"ColorHandleFiltered", L"Filtered processes", L"受句柄对象回调保护的进程。"),
    COLOR_ITEM(L"ColorUnknown", L"Untrusted DLLs and Services", L"未进行数字签名的服务和 DLL。"),
    COLOR_ITEM(L"ColorServiceDisabled", L"Disabled Services", L"已禁用的服务。"),
    //COLOR_ITEM(L"ColorServiceStop", L"Stopped Services", L"未运行的服务。")
    COLOR_ITEM(L"ColorEfficiencyMode", L"Power efficiency", L"设置节能模式的进程和线程。"),
};
...
{
    COLOR_ITEM(L"ColorCpuKernel", L"CPU 内核模式", L"CPU 内核模式"),
    COLOR_ITEM(L"ColorCpuUser", L"CPU 用户模式", L"CPU 用户模式"),
    ...
    COLOR_ITEM(L"ColorPowerUsage", L"电源用量", L"电源用量"),
    COLOR_ITEM(L"ColorTemperature", L"温度", L"温度"),
    COLOR_ITEM(L"ColorFanRpm", L"风扇转速 (转每分)", L"风扇转速 (转每分)"),
};
