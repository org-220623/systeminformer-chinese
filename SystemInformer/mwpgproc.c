    ...
    case MainTabPageInitializeSectionMenuItems:
        {
            ...
            PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_VIEW_HIDESIGNEDPROCESSES, L"隐藏已签名进程(&G)", NULL, NULL), startIndex + 1);
            PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_VIEW_HIDEMICROSOFTPROCESSES, L"隐藏系统进程(&S)", NULL, NULL), startIndex + 2);
            PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_VIEW_SCROLLTONEWPROCESSES, L"滚动到新进程(&L)", NULL, NULL), startIndex + 3);
            PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_VIEW_SHOWCPUBELOW001, L"显示 CPU 使用率低于 0.01% 的记录(&B)", NULL, NULL), startIndex + 4);
            ...
            PhInsertEMenuItem(menu, menuItem = PhCreateEMenuItem(0, ID_VIEW_ORGANIZECOLUMNSETS, L"组织列集(&Z)...", NULL, NULL), startIndex + 6);
            PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_VIEW_SAVECOLUMNSET, L"保存列集(&V)...", NULL, NULL), startIndex + 6);
            PhInsertEMenuItem(menu, columnSetMenuItem = PhCreateEMenuItem(0, 0, L"加载列集(&D)", NULL, NULL), startIndex + 7);
    ...
    )
{
    ...
    menuItem = PhCreateEMenuItem(0, ID_PROCESS_PRIORITY, L"优先级(&P)", NULL, NULL);
