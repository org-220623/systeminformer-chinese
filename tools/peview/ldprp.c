    ...
    )
{
    ...
    if (GuardFlags & IMAGE_GUARD_EH_CONTINUATION_TABLE_PRESENT_V1 || GuardFlags & IMAGE_GUARD_EH_CONTINUATION_TABLE_PRESENT_V2)
        PhAppendStringBuilder2(&stringBuilder, L"EH 延续表, ");
    ...
    case WM_INITDIALOG:
        {
            ...
            { \
                ...
                { \
                    ADD_VALUE(L"Guard EH 延续表", PhaFormatString(L"0x%Ix", (Config)->GuardEHContinuationTable)->Buffer); \
                    ADD_VALUE(L"Guard EH 延续表条目计数", PhaFormatUInt64((Config)->GuardEHContinuationCount, TRUE)->Buffer); \
                } \
            }
